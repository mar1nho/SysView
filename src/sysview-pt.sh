...
install_requirements() {
    ...
    if [ "$pkg_manager" = "unknown" ]; then
        echo -e "${log_prefix}❌ Gerenciador de pacotes não identificado.${NC}"
        echo -e "${log_prefix}Por favor, instale speedtest-cli manualmente.${NC}"
        return 1
    fi
    
    echo -e "${log_prefix}📦 Detectado gerenciador de pacotes: $pkg_manager${NC}"
    echo ""
    ...
    case $escolha in
        ...
        2)
            ...
            if [ $? -eq 0 ]; then
                ...
                echo -e "${log_prefix}❌ Falha na instalação.${NC}"
                return 1
            fi
            ;;
        *)
